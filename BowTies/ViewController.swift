...
class ViewController: UIViewController {
 ...
 var currentBowTie: BowTies!
  ...
  //2
    let request: NSFetchRequest<BowTies> = BowTies.fetchRequest()
    let firstTitle = segmentedControl.titleForSegment(at: 0)!
    request.predicate = NSPredicate(
      format: "%K = %@",
      argumentArray: [#keyPath(BowTies.searchKey), firstTitle])
    do {
  ...
  @IBAction func segmentedControl(_ sender: UISegmentedControl) {
    ...
    guard let selectedValue = sender.titleForSegment(
      ...
      request.predicate = NSPredicate(
        format: "%K = %@",
        argumentArray: [#keyPath(BowTies.searchKey), selectedValue])
      do {
        let results =  try managedContext.fetch(request)
        currentBowTie =  results.first
  ...
  func insertSampleData() {
      let fetch: NSFetchRequest<BowTies> = BowTies.fetchRequest()
  ...
  return
    ...
    for dict in dataArray {
      let entity = NSEntityDescription.entity(
        forEntityName: "BowTies",
        in: managedContext)!
      let bowtie = BowTies(entity: entity,
